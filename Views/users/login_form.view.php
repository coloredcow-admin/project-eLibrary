<div class="border border-secondary p-4 rounded bg-white col-sm-6 col-9 col-lg-12 mx-auto">
	<form method="POST" action="/login" onsubmit="return (checkFieldEmail('emailid') &&checkFieldPassword('password'))">
		...
		<small class="form-text text-muted text-right"><a href='javascript:void(0)'data-toggle="modal" data-target="#resetPasswordModal" title='Click here to get password reset link.' class="cust-link-primary">Forgot Password?</a></small>
		<button class="btn btn-cust-primary btn-block mt-2" type="submit">Login</button>
		<div class="row mx-1 my-1">
			<hr class="d-inline col">
			<p class="text-muted text-center d-inline col-2 pt-1 mb-0">or</p>
			<hr class="d-inline col">
		</div>       
		<button class="btn btn-cust-outline-light btn-block my-0" onclick="window.location='<?=$loginURL?>'"><img src="../resources/images/google_logo.jpg" class="d-inline rounded-circle mx-auto my-0 py-0"  alt="Login with Google" height="25"> &nbsp;Google Login
		</button>
		<div class="text-center mt-3 pl-2 mb-0">
			New User? <a class="cust-link-primary" href="/index?register=1">Join here</a>	
		</div>
	</form>         
</div>
</form>
